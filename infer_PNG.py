...
def main():
    ...
    for image_path in image_paths:
        ...
        inputIm = low_res.astype(np.float32)
        ...
        # np.float32(low_res)
        # print (low_res.dtype)
        inputIm = inputIm / 65535.0
        print (np.amax(inputIm))
        # print (low_res.dtype)
        # # np.float32(low_res)

        # # Convert to RGB (opencv uses BGR as default but the images are usually in RGB format - so we change the order of colors)
        # inputIm = cv2.cvtColor(inputIm, cv2.COLOR_BGR2RGB)



        # # Get super resolution image
        sr = model.predict(np.expand_dims(inputIm, axis=0))[0]
        ...
        # inputIm = cv2.cvtColor(inputIm, cv2.COLOR_RGB2BGR)
